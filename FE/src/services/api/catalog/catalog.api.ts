...
export interface CatalogAttribute { definitionId: string; name: string; dataType: 'TEXT'|'NUMBER'|'SELECT_ONE'|'SELECT_MULTI'; valueText?: string; valueNumber?: number; unit?: string; optionValues?: string[]; optionIds?: string[] }
...
export interface CatalogSummary { id: string; sellerId?: string; name: string; status: string; category?: { id:string; name:string; slug?:string }; minPrice?: number; maxPrice?: number; totalQuantity:number; activeVariantCount:number; thumbnailUrl?:string; attributePreview?: CatalogAttribute[]; axisPreview?: CatalogAxis[] }
export interface CatalogDetail { id:string; code:string; sellerId?:string; name:string; description?:string; status:string; category?:{id:string;name:string;slug?:string}; productImages:Array<{id:string;url:string;displayOrder:number}>; attributes:CatalogAttribute[]; variantAxes:CatalogAxis[]; variants:CatalogVariant[] }
const base='/api/v1/permitall'
