...
- Sua ngay 2026-08-11 cho user search san pham nhanh: them Elasticsearch dev single-node trong `backend-microservice/docker-compose.yml`, them `spring-boot-starter-data-elasticsearch` cho `catalog-service`, va them API rieng cho man user `GET /api/catalog/user/products/search?q=&category=&minPrice=&maxPrice=` qua gateway. Dong bo index `products` chuyen sang Outbox Pattern + Debezium CDC: admin product/product-detail ghi MySQL va insert `outbox` trong cung transaction, Debezium chi doc `ecommerce_catalog.outbox`, publish topic `outbox.event.Product`, Elasticsearch Sink ghi index/alias `products`. API admin san pham van query MySQL/JPA, khong doi sang Elasticsearch.
- Sua tiep ngay 2026-08-11 cho Debezium Outbox connector: bat `table.expand.json.payload=true` de Kafka value la JSON object cho Elasticsearch Sink, bat `route.tombstone.on.empty.payload=true` de `ProductDeleted` voi payload null xoa document theo key; JSON connector/mapping parse PASS, `docker compose -f backend-microservice\docker-compose.yml config` PASS, `:catalog-service:build --no-daemon --max-workers=1` PASS.
- Sua tiep ngay 2026-08-11 cho endpoint FE dang goi `GET /api/v1/permitall/san-pham/get-all/danh-sach-san-pham`: khi co `q`, `catalog-service` dung Elasticsearch index `products` de lay product id phu hop, sau do van enrich/paginate theo response legacy (`tenSanPham`, `giaBan`, `mauSac[]`, `kichCo[]`, `dotGiamGia`). Neu Elasticsearch chua san sang hoac index chua co du lieu thi fallback ve loc MySQL/JPA de trang san pham khong bi 500. `:catalog-service:compileJava -x :common-lib:jar --no-daemon --max-workers=1` PASS; runtime `GET http://localhost:8080/api/v1/permitall/san-pham/get-all/danh-sach-san-pham?page=1&size=12&sortBy=createdAt_desc&q=running` PASS va tra `Adidas Run Demo` qua Elasticsearch document field `category=Running`.
- Sua ngay 2026-08-11 cho man admin san pham bi trang: `GET /api/v1/admin/san-pham` duoc tach lai dung `getAdminAll()` va projection admin (`ten`, `tenThuongHieu`, `tenDanhMuc`, `tongSP`, `status`) thay vi public shape (`tenSanPham`, `thuongHieu`, `giaBan`).
- Sua ngay 2026-08-11 cho man admin san pham update/doi trang thai 500: log catalog-service bao `Table 'ecommerce_catalog.outbox' doesn't exist` khi `POST /api/v1/admin/san-pham` va `PUT /api/v1/admin/san-pham/{id}/change-status` ghi Product outbox event. Them `OutboxSchemaInitializer` tao idempotent bang `outbox` thuoc catalog-service cho DB local cu, va them `globally_quoted_identifiers_skip_column_definitions` de Hibernate khong sinh sai DDL cot JSON thanh `` `json` ``. Doi chieu AGENTS.md muc 5: khong them bang/join DB cheo, `outbox` la bang catalog-domain cho CDC/search; `:catalog-service:build --no-daemon --max-workers=1` PASS; runtime direct `POST /api/v1/admin/san-pham` PASS, direct va gateway `PUT /api/v1/admin/san-pham/{id}/change-status` PASS het 500.
- Doi chieu AGENTS.md muc 5 cho search Elasticsearch/Outbox: khong them bang/join DB cheo; `outbox` nam trong DB catalog-service va chi chua event public cho product-domain; Elasticsearch chi la read model user search, nguon ghi chinh van la MySQL catalog; admin CRUD/list giu MySQL/JPA; Kafka KRaft khong dung Zookeeper; Debezium chi doc bang `outbox`, khong doc truc tiep bang product. `:catalog-service:compileJava -x :common-lib:jar --no-daemon --max-workers=1` PASS va `:catalog-service:build --no-daemon --max-workers=1` PASS ngay 2026-08-11; `docker compose -f backend-microservice\docker-compose.yml config` PASS, co canh bao Docker config local access denied khong chan render config.
...
- Compose gom MySQL 8.4, Kafka KRaft, Kafka UI, Elasticsearch dev single-node, Kafka Connect, Eureka, Gateway va cac service app; khong dung Zookeeper.
