-- P1 schema verification. Run after the additive schema script.

SELECT 'legacy_table_count' AS check_name, COUNT(*) AS actual_value, 0 AS expected_value
FROM information_schema.tables
WHERE table_schema = DATABASE()
  AND table_name IN ('brand', 'color', 'material', 'origin', 'size', 'sole_type', 'product_attribute_value_option');

SELECT 'target_table_count' AS check_name, COUNT(*) AS actual_value, 14 AS expected_value
FROM information_schema.tables
WHERE table_schema = DATABASE()
  AND table_name IN (
...
SELECT 'required_column_count' AS check_name, COUNT(*) AS actual_value, 6 AS expected_value
...
SELECT 'product_variant_unique_combination' AS check_name, COUNT(*) AS actual_value, 2 AS expected_value
...
SELECT 'foreign_key_count' AS check_name, COUNT(*) AS actual_value, 19 AS expected_value
